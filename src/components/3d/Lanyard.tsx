...
export default function Lanyard({ position = [0, 0, 15], gravity = [0, -40, 0], fov = 12, transparent = true, scale = 3.5 }: LanyardProps) {
  return (
    <div className="lanyard-wrapper w-full h-full absolute inset-0" style={{ pointerEvents: 'auto' }}>
      <Canvas
...
function Band({ maxSpeed = 50, minSpeed = 0, scale = 2.5 }) {
  ...
  return (
    ...
    <RigidBody ref={fixed} type="fixed" colliders="ball" canSleep angularDamping={4} linearDamping={4} />
    <RigidBody position={[0.5, 0, 0]} ref={j1} type="dynamic" colliders="ball" canSleep angularDamping={4} linearDamping={4}>
          <BallCollider args={[0.1]} />
        </RigidBody>
    <RigidBody position={[1, 0, 0]} ref={j2} type="dynamic" colliders="ball" canSleep angularDamping={4} linearDamping={4}>
          <BallCollider args={[0.1]} />
        </RigidBody>
    <RigidBody position={[1.5, 0, 0]} ref={j3} type="dynamic" colliders="ball" canSleep angularDamping={4} linearDamping={4}>
          <BallCollider args={[0.1]} />
        </RigidBody>
    <RigidBody position={[2, 0, 0]} ref={card} type={dragged ? 'kinematicPosition' : 'dynamic'} colliders={false} canSleep angularDamping={4} linearDamping={4}>
      ...
      <mesh ref={band}>
        ...
        <meshLineMaterial 
          ...
          map-repeat={[-4, 1]}
          depthTest={false}
          resolution={new THREE.Vector2(isSmall ? 1000 : 2000, isSmall ? 2000 : 1000)}
        />
      </mesh>
    </>
  );
